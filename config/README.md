...
| `draw_rules` | bool | `true` | End an episode as a draw on threefold repetition, 50 moves without a capture or a pawn move, insufficient material, or stalemate (`core/draw.py`) instead of shuffling to `max_steps_per_episode`. The draw carries **no** reward of its own. `false` restores the old behaviour. |
