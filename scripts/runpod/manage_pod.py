...
A ``.env`` *file* in the current directory is also loaded automatically -- but note
that in this repo ``.env`` at the root is the Python virtualenv *directory*, not a
dotenv file, so prefer setting ``RUNPOD_API_KEY`` as an environment variable.
