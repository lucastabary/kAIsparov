...
| `agents/` | Policies with `select_move(game)`: `RandomAgent`, `MaterialAgent`, `NeuralAgent`, `MinimaxAgent` (alpha-beta over any `analysis/evaluators` evaluator — material, heuristic, a network's critic via `on_model`), and `Fallible` (any agent, plus random moves). |
