...
def test_runmanager_writes_artifacts_and_registry_reads_them(tmp_path):
    ...
    rm.save_checkpoint(model, 2, {"elo_vs_random": 10.0})
    rm.log_eval(4, {"elo_vs_random": 30.0})
    rm.save_checkpoint(model, 4, {"elo_vs_random": 30.0})
    ...
    assert not (rm.dir / "checkpoints" / "best.pth").exists()  # no "best" in this project
    ...
    assert "best_checkpoint" not in run
